...
export const LocalTabsProvider: ParentComponent = (props) => {
  ...
  const close = (id: string) => {
    ...
    if (isPendingTab(id)) {
      // testagent_change: removed isSubmitting check - not available in testagent
      if (isPendingSend(id)) discardPendingDraft(id)
  ...
  const closeOthers = (id: string) => {
    ...
    for (const pending of removed) {
      // testagent_change: removed isSubmitting check - not available in testagent
      if (isPendingSend(pending)) discardPendingDraft(pending)
    }
