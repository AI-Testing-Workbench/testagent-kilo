          ...
          <Show when={assistantMessages().length > 0}>
            <div class="vscode-session-turn-assistant">
              <For each={assistantMessages()}>
                {(msg) => (
                  <>
                    <AssistantMessage message={msg} showAssistantCopyPartID={showAssistantCopyPartID()} />
                    <Show when={msg.error && msg.error.name !== "MessageAbortedError"}>
                      <div style={{ position: "relative", margin: "8px 0", padding: "8px" }}>
                        <ErrorDisplay error={msg.error!} onLogin={server.startLogin} />
                        <div style={{ marginTop: "8px" }}>
                          <span
                            onClick={() => session.continueTask()}
