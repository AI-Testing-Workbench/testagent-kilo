...
// Align with upstream: lineDiffType is "word-alt" for split diffs, "none" otherwise.
// Upstream imports this from `@kilocode/kilo-ui/pierre`, but that export is not
// actually present in the built package, so we define it locally to avoid the
// missing-export build error while keeping the same runtime behavior.
const LINE_DIFF_TYPE: "none" | "word-alt" = "none"
