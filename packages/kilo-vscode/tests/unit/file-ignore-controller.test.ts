...
describe("FileIgnoreController", () => {
  describe("when .testagentignore exists", () => {
    ...
    it("applies only .testagentignore patterns", async () => {
      ...
      expect(controller.getInstructions()).toContain(".testagentignore") // testagent_change
    })

    it("does NOT block .env files unless explicitly listed", async () => {
      const workspace = await createTempWorkspace()
      await fs.writeFile(path.join(workspace, ".testagentignore"), "dist/\n") // testagent_change
    ...
    it("does NOT apply .gitignore patterns", async () => {
      ...
      // .testagentignore pattern should apply // testagent_change
  ...
  describe("when no .testagentignore exists (fallback)", () => {
    ...
    it("blocks .env files via hardcoded sensitive patterns", async () => {
      const workspace = await createTempWorkspace()
      // No .testagentignore, no .gitignore // testagent_change
