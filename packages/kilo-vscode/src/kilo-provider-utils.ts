...
function safeStringify(value: unknown): string | undefined {
  try {
    ...
    console.warn("[TestAgent New] getErrorMessage: JSON.stringify failed", err)
  }
  return undefined
}
...
export async function runWithMessageConfirmation<T>(
  ...
  try {
    ...
    if (await state.wait(id)) {
      console.warn(`[TestAgent New] ${label} ended after server accepted it; ignoring transport error`, {
...
export type WebviewMessage =
  ...
  | { type: "sessionInfo"; sessionID?: string; message: string } // testagent_change
  | null
...
export function mapSSEEventToWebviewMessage(event: Event, sessionID: string | undefined): WebviewMessage {
  switch (event.type) {
    ...
    // testagent_change start - handle session.info events
    case "session.info": {
      return {
        type: "sessionInfo",
        sessionID: event.properties.sessionID,
