...
export async function handlePermissionResponse(
  ...
  if (!target) {
    console.error("[TestAgent]  No sessionID for permission response")
    ctx.postMessage({ type: "permissionError", permissionID: permissionId })
    return
  }
  ...
  try {
    ...
    console.error("[TestAgent]  Failed to respond to permission:", error)
    ctx.postMessage({ type: "permissionError", permissionID: permissionId })
  }
}
...
export async function fetchAndSendPendingPermissions(ctx: PermissionContext): Promise<void> {
  ...
  try {
    ...
    console.error("[TestAgent]  Failed to fetch pending permissions:", error)
  }
}
