...
export class SdkSSEAdapter {
  ...
  connect(): void {
    if (this.abortController) {
      console.log("[TestAgent] SSE: ⚠️ Already connected, skipping")
      return
    }
    ...
    void this.consumeLoop(this.abortController.signal).catch((err) => {
      console.error("[TestAgent] SSE: Unhandled error in consumeLoop:", err)
   ...
   */
  disconnect(): void {
    console.log("[TestAgent] SSE: 🔌 disconnect() called")
  ...
  reconnect(): void {
    if (!this.attemptController) {
      console.log("[TestAgent] SSE: ⚠️ reconnect() called but no active attempt")
      return
    }
    console.log("[TestAgent] SSE: 🔄 reconnect() — aborting current attempt")
  ...
  private async consumeLoop(signal: AbortSignal): Promise<void> {
    while (!signal.aborted) {
      ...
      try {
        console.log("[TestAgent] SSE: 🎬 Calling SDK global.event()...")
        ...
        for await (const event of events.stream) {
          ...
          if (type !== "server.heartbeat" && type !== "message.part.delta") {
            console.log("[TestAgent New] SSE: 📨 Event:", type, globalEvent.payload)
          }
          this.notifyEvent(globalEvent.payload as Event)
        }

        console.log("[TestAgent] SSE: 📭 Stream ended normally")
      } catch (error) {
        ...
        if (!aborted) {
          console.error("[TestAgent] SSE: ❌ Stream error:", error)
          this.notifyError(error instanceof Error ? error : new Error(String(error)))
        }
      } finally {
  ...
  private resetHeartbeat(attempt: AbortController): void {
    ...
    this.heartbeatTimer = setTimeout(() => {
      console.log("[TestAgent] SSE: ⏰ Heartbeat timeout — aborting stale connection")
      attempt.abort()
  ...
  private notifyEvent(event: Event): void {
    for (const handler of this.handlers) {
      try {
        ...
        console.error("[TestAgent] SSE: Error in event handler:", error)
      }
    }
  }
  ...
  private notifyError(error: Error): void {
    for (const handler of this.errorHandlers) {
      try {
        ...
        console.error("[TestAgent] SSE: Error in error handler:", err)
      }
    }
  }
  ...
  private notifyState(state: "connecting" | "connected" | "disconnected"): void {
    for (const handler of this.stateHandlers) {
      try {
        ...
        console.error("[TestAgent] SSE: Error in state handler:", error)
      }
    }
  }
}
