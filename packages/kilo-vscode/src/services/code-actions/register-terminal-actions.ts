...
export function registerTerminalActions(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("testagent.new.terminalAddToContext", async (args: any) => {
    ...
    vscode.commands.registerCommand("testagent.new.terminalFixCommand", async (args: any) => {
      let content = args?.selection as string | undefined
      if (!content) {
        content = (await getTerminalContents(1)).content
      }
    ...
    vscode.commands.registerCommand("testagent.new.terminalExplainCommand", async (args: any) => {
      let content = args?.selection as string | undefined
      if (!content) {
        content = (await getTerminalContents(1)).content
      }
